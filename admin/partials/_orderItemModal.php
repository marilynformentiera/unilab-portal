                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        $drugsId = $myrow['drugsId'];
                                        $itemQuantity = $myrow['itemQuantity'];
                                        
                                        $itemsql = "SELECT * FROM `drugs` WHERE drugsId = $drugsId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $drugsName = $itemrow['drugsName'];
                                        $drugsPrice = $itemrow['drugsPrice'];
                                        $drugsDesc = $itemrow['drugsDesc'];
                                        $drugsCategorieId = $itemrow['drugsdivisionId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="/unilab-portal/img/drugs-'.$drugsId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> '.$drugsName. '</h5><span class="text-muted font-weight-normal font-italic d-block">₱ ' .$drugsPrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>
